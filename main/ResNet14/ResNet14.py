...
def cfr10_ResNet14S_create(
  ...
  x = tf.keras.layers.Conv2D(8, 3, padding='same', kernel_initializer=initializer, kernel_regularizer=l2_reg)(inputs)
  x = tf.keras.layers.Activation('relu')(x)
  x = residual_block(x, initializer=initializer, regularizer=l2_reg, filters=8, num_res_blocks=num_res_blocks, downsampeling=False)
  x = residual_block(x, initializer=initializer, regularizer=l2_reg, filters=16, num_res_blocks=num_res_blocks, downsampeling=True)
...
epochs_train=160
lr_schedule = lrs_str_to_learning_rate("PW01x160", 1e-2, y_train.shape[0], batch_size)
