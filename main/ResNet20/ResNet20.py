...
def cfr10_ResNet20BN_create(
    ...
    x = tf.keras.layers.Conv2D(
        16, 3, padding='same', kernel_initializer=initializer, kernel_regularizer=l2_reg)(inputs)
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.Activation('relu')(x)
    x = residual_blockBN(x, initializer=initializer, regularizer=l2_reg,
                         filters=16, num_res_blocks=num_res_blocks, downsampeling=False)
    x = residual_blockBN(x, initializer=initializer, regularizer=l2_reg,
                         filters=32, num_res_blocks=num_res_blocks, downsampeling=True)
    x = residual_blockBN(x, initializer=initializer, regularizer=l2_reg,
                         filters=64, num_res_blocks=num_res_blocks, downsampeling=True)
...
lr_schedule = lrs_str_to_learning_rate(
    "PW01x160", 1e-3, y_train.shape[0], batch_size)
